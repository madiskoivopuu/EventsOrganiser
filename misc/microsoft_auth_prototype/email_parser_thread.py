import json, langdetect, time, requests
...
def read_user_emails(access_token):
    ...
    if(response.status_code != 200):
        raise RuntimeError(f"email fetch unexpected status code {response.status_code}")
    
    return response.json()

def parser_loop(msal_app):
    while True:
        time.sleep(3)
        ...
        for account in accounts:
            result = msal_app.acquire_token_silent(["Mail.ReadBasic", "Mail.Read"], account=account)
            emails = read_user_emails(result["access_token"])
            eng_emails = []
...
Event Start Date - date formatted in hh:mm DD/MM/YYYY using the ISO 8601 convention; hh:mm can be missing if no time of day is mentioned; N/A if no start date is mentioned or an event is a deadline; it is possible that the date is described with words like 'tomorrow', 'in 3 days' etc and you will need to deduce the start date based on the email sending date, which is **CHANGE THIS**
Event End Date - date formatted in hh:mm DD/MM/YYYY using the ISO 8601 convention; hh:mm can be missing if no time of day is mentioned; N/A if no end date is mentioned; some details might be missing from the end date and you will need do deduce it from the email sending date, which is **CHANGE THIS**
